...
static ERRORTYPE loadSampleUvc2VoConfig(SampleUvc2VoConfig *pConfig, const char *conf_path)
{
    ...
    if(!conf_path) {
        ...
        //pConfig->mPicFormat = GetConfParaInt(&stConfParser, SAMPLE_UVC2JPEG_KEY_PIC_FORMAT, V4L2_PIX_FMT_NV21);
        pConfig->mPicFormat = V4L2_PIX_FMT_YUYV;
        pConfig->mDisplayWidth = GetConfParaInt(&stConfParser, SAMPLE_UVC2VO_KEY_DISPLAY_WIDTH, 640);
        pConfig->mDisplayHeight = GetConfParaInt(&stConfParser, SAMPLE_UVC2VO_KEY_DISPLAY_HEIGHT, 480);
...
main(int argc, char **argv)
{
    ...
    if(SUCCESS == AW_MPI_UVC_GetBrightness(stContext.mConfigParam.mDevName, &BrightnessValue)) {
        alogw("the brightness = %d", BrightnessValue);
    ...
    if(SUCCESS == AW_MPI_UVC_GetContrast(stContext.mConfigParam.mDevName, &ContrastValue)) {
        alogw("the Contrast = %d", ContrastValue);
    ...
    if(SUCCESS == AW_MPI_UVC_GetSaturation(stContext.mConfigParam.mDevName, &SaturationValue)) {
        alogw("the Saturation = %d", SaturationValue);
    ...
    if(SUCCESS == AW_MPI_UVC_GetSharpness(stContext.mConfigParam.mDevName, &SharpnessValue)) {
        alogw("the Sharpness = %d", SharpnessValue);
