...
******************************************************************************/
void *HCNTProcessThread(void *pArg)
{
    int iRet;
    CVE_HCNT_CONF_INFO_S *pstConfInfo = (CVE_HCNT_CONF_INFO_S *)pArg;
...
#endif
        eProcessRet = AW_AI_CVE_HCNT_Process(pstConfInfo->psHCNTHd, &stImage, iTimeStamp, pstHCNTResult);
#ifdef TIME_TEST
...
static ERRORTYPE InitVippDevice(CVE_HCNT_CONF_INFO_S *pConfInfo)
{
    int iRet = SUCCESS;
...
static ERRORTYPE UnInitVippDevice(CVE_HCNT_CONF_INFO_S *pConfInfo)
{
...
//        alogd("parse cmdline error, may use default configuration.\n");
    ...
    iRet = InitVippDevice(&stHCNTConfInfo);
    if (iRet < 0) {
...
#define ISP_RUN 1
    ...
    if (stHCNTConfInfo.iDev == 0 || stHCNTConfInfo.iDev == 2) {
        iIspDev = 1;
    } else if (stHCNTConfInfo.iDev == 1 || stHCNTConfInfo.iDev == 3) {
        iIspDev = 0;
    }
    AW_MPI_ISP_Init();
    ...
    int iViDev = stHCNTConfInfo.iDev;
    int iViChn = 0;
    ...
    stHCNTConfInfo.iChn = iViChn;
    iRet = pthread_create(&tid, NULL, HCNTProcessThread, &stHCNTConfInfo);
    if (iRet != 0) {
...
init_hcnt_module_err:
    UnInitVippDevice(&stHCNTConfInfo);
init_vipp_err:
    if (0 == iRet) {
        printf("sample_vi2HCNT exit!\n");
    }
    return iRet;
}
