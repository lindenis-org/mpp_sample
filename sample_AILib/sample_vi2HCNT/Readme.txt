...
sample_vi2HCNT测试流程：
	从Vipp设备获取视频帧，送到hcnt库中进行分析处理得到相关结果（包括目标个数，目标位置等等），输出每个目标的位置以及矩形框等。
...
读取测试参数的流程：
    ...
    1. sample_vi2HCNT.conf，测试参数都写在该文件中：
        启动sample_vi2HCNT时，在命令行参数中给出sample_vi2HCNT.conf的具体路径，sample_vi2HCNT会读取sample_vi2HCNT.conf，完成参数解析。
    2. 命令行输入：
        "exec [-h|--help] [-p|--path]\n"
        "   <-h|--help>: 打印帮助信息\n"
...
从命令行启动sample_vi2HCNT的指令：
./sample_vi2HCNT -h
./sample_vi2HCNT --path /mnt/extsd/sample_vi2HCNT.conf || ./sample_vi2HCNT -x 640 -y 360 -e nv21 -d 3 -t 4
