...
sample_vi2VLPR测试流程：
	从Vipp设备获取视频帧，送到VLPR库中进行分析处理得到相关结果（包括车牌的号码、颜色、倾斜度等等），输出车牌识别号码、车牌类型等等。
...
读取测试参数的流程：
    ...
    1. sample_vi2VLPR.conf，测试参数都写在该文件中：
        启动sample_vi2VLPR时，在命令行参数中给出sample_vi2VLPR.conf的具体路径，sample_vi2VLPR会读取sample_vi2VLPR.conf，完成参数解析。
    2. 命令行输入：
        "exec [-h|--help] [-p|--path]\n"
        "   <-h|--help>: 打印帮助信息\n"
...
从命令行启动sample_vi2VLPR的指令：
./sample_vi2VLPR -h
./sample_vi2VLPR -path /mnt/extsd/sample_cve_vlpr.conf || ./sample_vi2VLPR -x 1280 -y 720 -e nv21 -d 3 -t 4

测试时需要拷贝Binary文件夹到测试路径下面，程序默认打开程序所在文件夹下的Binary里面的model文件

测试参数的说明：
(1)pic_width ：指定原始视频文件的图像帧宽度
(2)pic_height：原始视频文件的图像帧高度
(3)pic_format: 原始视频文件的像素格式
(4)frame_rate: 帧率
