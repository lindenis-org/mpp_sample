DTCA: Dynamic Target Detection Algorithm - （动态）运动目标检测算法

编译此例程需要修改 middleware/config/mpp_config.mk 文件：MPPCFG_MOD := Y

sample_cve_MOD测试流程：
	从YUV文件取视频帧，送到dtca库中进行分析处理得到相关结果（包括目标的个数、位置等等）。
...
读取测试参数的流程：
    ...
    1. sample_cve_MOD.conf，测试参数都写在该文件中：
        启动sample_cve_MOD时，在命令行参数中给出sample_cve_MOD.conf的具体路径，sample_cve_MOD会读取sample_cve_MOD.conf，完成参数解析。
    2. 命令行输入：
        "exec [-h|--help] [-p|--path]\n"
        "   <-h|--help>: 打印帮助信息\n"
...
注意：分辨率要低于720P
从命令行启动sample_cve_MOD的指令：
./sample_cve_MOD -h
./sample_cve_MOD --path /mnt/extsd/sample_cve_MOD.conf || ./sample_cve_MOD -x 640 -y 360 -e nv21 -t 4

测试时需要拷贝DTCAParam文件夹到测试路径下面，程序默认打开程序所在文件夹下的DTCAParam里面的xml文件
