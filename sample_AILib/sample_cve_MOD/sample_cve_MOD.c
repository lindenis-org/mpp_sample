...
******************************************************************************/
int DTCAProcessImage(AW_IMAGE_S *pstImage, CVE_DTCA_CONF_INFO_S *pstConfInfo)
{
    int iRet;
    int iFd;
...
#endif
        eProcessRet = AW_AI_CVE_DTCA_Process(pstConfInfo->psDTCAHd, pstImage, iTimeStamp, pstDTCAResult);
#ifdef TIME_TEST
...
//        alogd("parse cmdline error, may use default configuration.\n");
    ...
    ******************************************/
    ...
    stImage.mPixelFormat = stDTCAConfInfo.ePixFmt;
    stImage.mWidth       = stDTCAConfInfo.iPicWidth;
    stImage.mHeight      = stDTCAConfInfo.iPicHeight;
    stImage.mStride[0]   = stDTCAConfInfo.iPicWidth;
...
//  iRet = ion_flushCache(stImage.mpVirAddr[0], iYSize);
    ...
    /******************************************
    step 3 : DTCA algo process
    ******************************************/
    iRet = DTCAProcessImage(&stImage, &stDTCAConfInfo);
    if (iRet < 0) {
