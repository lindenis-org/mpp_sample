sample_vi2EVE测试流程:
	从vipp获取视频数据送入到eve库中进行分析处理得到相关结果，并输出人脸识别的结果（数量、位置等）。
...
读取测试参数的流程：
	...
	1. sample_vi2EVE.conf，测试参数都写在该文件中：
		启动sample_vi2EVE时，在命令行参数中给出sample_vi2EVE.conf的具体路径，sample_vi2EVE会读取sample_vi2EVE.conf，完成参数解析。
	2. 命令行输入：
        "exec [-h|--help] [-p|--path]\n"
        "   <-h|--help>: 打印帮助信息\n"
...
从命令行启动sample_vi2EVE的指令：
	./sample_vi2EVE -p /mnt/extsd/sample_vi2EVE.conf || ./sample_vi2EVE -x 640 -y 360 -f 25 -e nv21 -d 3
