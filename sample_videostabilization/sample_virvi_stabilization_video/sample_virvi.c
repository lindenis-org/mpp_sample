...
static ERRORTYPE loadSampleVirViConfig(SampleVirViConfig *pConfig, const char *conf_path)
{
    ...
    pConfig->svMode = GetConfParaInt(&stConfParser, SAMPLE_VirVi_StaVidMode, 0);
...
static void *GetCSIFrameThread(void *pArg)
{
    ...
    while (GetFrameCount != j) {
        ...
        // printf("Cap threadid=0x%lx, ViDev=%d,VirVi=%d,mpts=%lld;\r\n", pCap->thid, ViDev, ViCh, pCap->pstFrameInfo.VFrame.mpts);
...
static void *SubGetCSIFrameThread(void *pArg)
{
    VI_DEV ViDev;
    VI_CHN ViCh;
    int ret = 0;
...
int main(int argc, char *argv[])
{
    ...
    while (count != AutoTestCount) {
        ...
        system("cat /proc/meminfo | grep CmaFree");
        ...
        stAttr.wdr_mode = 0;
        stAttr.antishake_enable = 1;
        stAttr.antishake_attr.frame_width = stContext.mConfigPara.PicWidth;//2208;//1920;
        stAttr.antishake_attr.frame_width_strie = stContext.mConfigPara.PicWidth;//2208;//1920;
        stAttr.antishake_attr.frame_height = stContext.mConfigPara.PicHeight;//1242;//1080;
        stAttr.antishake_attr.frame_height_strie = stContext.mConfigPara.PicHeight;//1242;//1080;
        stAttr.antishake_attr.number_of_input_buffers = 5;
        stAttr.antishake_attr.number_of_output_buffers = 10;
        stAttr.antishake_attr.operation_mode = stContext.mConfigPara.svMode; // 13;//10;
        AW_MPI_VI_CreateVipp(vipp_dev);
        AW_MPI_VI_SetVippAttr(vipp_dev, &stAttr); // AW_MPI_VI_GetVippAttr(ViDev, pstAttr);
        AW_MPI_VI_EnableVipp(vipp_dev);

        sub_stAttr.type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
        sub_stAttr.memtype = V4L2_MEMORY_MMAP;
        sub_stAttr.format.pixelformat = stContext.mConfigPara.PicFormat; // V4L2_PIX_FMT_SBGGR12;
        sub_stAttr.format.field = V4L2_FIELD_NONE;
        sub_stAttr.format.width = 640;
        sub_stAttr.format.height = 480;
        sub_stAttr.nbufs = 5;
        sub_stAttr.nplanes = 2;
        sub_stAttr.fps = 30;
        sub_stAttr.capturemode = V4L2_MODE_VIDEO; // V4L2_MODE_VIDEO; V4L2_MODE_IMAGE; V4L2_MODE_PREVIEW
        sub_stAttr.use_current_win = 1;
        sub_stAttr.wdr_mode = 0;
        sub_stAttr.antishake_enable = 0;
        AW_MPI_VI_CreateVipp(1);
        AW_MPI_VI_SetVippAttr(1, &sub_stAttr); // AW_MPI_VI_GetVippAttr(ViDev, pstAttr);
        AW_MPI_VI_EnableVipp(1);
#define ISP_RUN 1
#if ISP_RUN
        AW_MPI_ISP_Init();
        ...
        for (virvi_chn = 0; virvi_chn < 1; virvi_chn++) {
            memset(&privCap[1][virvi_chn], 0, sizeof(VirVi_Cap_S));
            privCap[1][virvi_chn].Dev = 1;
            privCap[1][virvi_chn].Chn = virvi_chn;
            privCap[1][virvi_chn].s32MilliSec = 5000;  // 2000;
            if (0 == virvi_chn) { /* H264, H265, MJPG, Preview(LCD or HDMI), VDA, ISE, AIE, CVBS */
                ret = hal_virvi_start(1, virvi_chn, NULL); /* For H264 */
                if(ret != 0) {
                    printf("virvi start failed!\n");
                    result = -1;
                    goto _exit;
                }
                privCap[1][virvi_chn].thid = 0;
                ret = pthread_create(&privCap[1][virvi_chn].thid, NULL, SubGetCSIFrameThread, (void *)&privCap[1][virvi_chn]);
                if (ret < 0) {
                    alogd("pthread_create failed, Dev[%d], Chn[%d].\n", privCap[1][virvi_chn].Dev, privCap[1][virvi_chn].Chn);
                    continue;
                }
            }
        }
        ...
        for (virvi_chn = 0; virvi_chn < 1; virvi_chn++) {
            pthread_join(privCap[1][virvi_chn].thid, NULL);
        }
        ...
        AW_MPI_VI_DisableVipp(1);
        AW_MPI_VI_DestoryVipp(1);
        /* exit mpp systerm */
        ret = AW_MPI_SYS_Exit();
        if (ret < 0) {
