...
static void *GetEncoderFrameThread(void *pArg)
{
    ...
    while(count != EncoderCount) {
        ...
        if((ret = AW_MPI_VENC_GetStream(mVeChn,&VencFrame,4000)) < 0) { //6000(25fps) 4000(30fps)
            ...
            // printf("venc Y length:%d, C length:%d.\r\n", VencFrame.mpPack->mLen0, VencFrame.mpPack->mLen1);
...
int main(int argc, char *argv[])
{
    ...
    while(count != AutoTestCount) {
        ...
        stAttr.antishake_attr.number_of_output_buffers = 20;
        stAttr.antishake_attr.operation_mode = 11;
        ...
        SIZE_S videosize = {1680,944};//{stContext.mConfigPara.DestWidth, stContext.mConfigPara.DestHeight};
